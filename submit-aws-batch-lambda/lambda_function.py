...
API_URL = os.environ.get("API_URL")  # 추가: API URL 환경변수
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        for record in event["Records"]:
            ...
            job_responses = submit_batch_jobs_for_restaurants(saved_restaurants, bucket_name, object_key)
...
def post_restaurant_to_api(restaurant: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            logger.error(
                f"API 저장 실패: {restaurant.get('placeId', 'unknown')} - status {response.status_code}, response: {response.text}"
            )
            return None
    except Exception as e:
        logger.error(
...
def submit_batch_jobs_for_restaurants(restaurants: List[Dict[str, Any]], bucket_name: str, object_key: str) -> List[Dict[str, Any]]:
    job_responses = []
    for restaurant in restaurants:
        try:
            ...
            logger.error(f"Error submitting batch job for placeId {restaurant.get('placeId', 'unknown')}: {str(e)}")
